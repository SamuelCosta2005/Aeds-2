...
class Q1 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while(esc.hasNextInt()){
            Cadastro[i]=PesquisaBin(0, array.length-1, esc.nextInt(), array);
            i++;
        }
        esc.next();
        Lista a=new Lista(Cadastro,i);
        int d=esc.nextInt();
        for(;d>0;d--){
            String c=esc.next();
            if(c.equals("II")){
                a.InserirInicio(PesquisaBin(0, array.length-1, esc.nextInt(), array));
            }
            else if(c.equals("IF")){
                a.inserirFim(PesquisaBin(0, array.length-1, esc.nextInt(), array));
            }
            else if(c.equals("I*")){
                a.inserirPos(esc.nextInt(), PesquisaBin(0, array.length-1, esc.nextInt(), array));
            }
            else if(c.equals("RI")){
                System.out.println("(R) "+a.RemoveInicio().Nome);
            }
            else if(c.equals("RF")){
                System.out.println("(R) "+a.RemoveFim().Nome);
            }
            else if(c.equals("R*")){
                System.out.println("(R) "+a.removePos(esc.nextInt()).Nome);
            }
        }
        for(int k=0;k<a.n;k++){
            System.out.print("["+k+"] ");
