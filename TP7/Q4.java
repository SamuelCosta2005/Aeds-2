...
class ArvoreRB {
    ...
    public void inserir(Game x) throws Exception {
        raiz = inserir(raiz, x);
        if (raiz != null) raiz.cor = false; // raiz sempre preta
    }

    private Game inserir(Game i, Game x) throws Exception {
        if (i == null) {
            // Inserir o nó x como novo nó (se preferir, clone os dados para evitar reuso de objetos)
            x.cor = true; // novo nó vermelho
            x.esq = null;
            x.dir = null;
            return x;
        }
        // Regras de balanceamento LLRB
        if (isVermelho(i.dir) && !isVermelho(i.esq)) {
            i = rotacaoEsquerda(i);
        }
        if (isVermelho(i.esq) && isVermelho(i.esq.esq)) {
            i = rotacaoDireita(i);
        }
        if (isVermelho(i.esq) && isVermelho(i.dir)) {
            flipCores(i);
        }
        int cmp = x.Nome.compareTo(i.Nome);
        Q4.numcomp++; // contar a comparação feita
        if (cmp < 0) {
    ...
    private boolean isVermelho(Game n) {
        return n != null && n.cor; // true = vermelho
    }

    private Game rotacaoEsquerda(Game h) {
        Game x = h.dir;
        h.dir = x.esq;
        x.esq = h;
        x.cor = h.cor;
        h.cor = true; // h fica vermelho
        return x;
    }

    private Game rotacaoDireita(Game h) {
        Game x = h.esq;
        h.esq = x.dir;
        x.dir = h;
        x.cor = h.cor;
    ...
    private void flipCores(Game h) {
        // segurança: só flip se filhos existirem (normalmente esta função é chamada
        // apenas quando ambos filhos são não nulos e vermelhos)
        if (h == null) return;
    ...
    public boolean pesquisar(Game i, String nome) {
        boolean resp = false;
        if (i == null) {
            resp=false;
        }
        else if (i.Nome.equals(nome)) {
            resp = true;
        }
