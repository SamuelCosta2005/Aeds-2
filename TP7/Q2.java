...
class ArvSec{
    ...
    public No inserir(No i, Game x) throws Exception{
        if(i==null){
            i=new No(x);
        }
        else if(x.Nome.compareTo(i.elem.Nome) < 0){
            Q2.numcomp++;
    ...
    public boolean pesquisar(No i, String nome) {
        boolean resp=false;
        if(i!=null){
            if(i.elem.Nome.equals(nome)){
                resp=true;
            }
            else if(nome.compareTo(i.elem.Nome) < 0){
...
class ArvDeArv {
    ...
    public ArvDeArv() {
        this.raiz = null;
        // Constrói a árvore principal com a estrutura fixa
        construirArvorePrincipal();
    }

    private void construirArvorePrincipal() {
        // Insere os nós na ordem especificada
        int[] chaves = {7, 3, 11, 1, 5, 9, 13, 0, 2, 4, 6, 8, 10, 12, 14};
        for (int chave=0;chave< chaves.length;chave++) {
            raiz = inserirEstrutura(raiz, chaves[chave]);
        }
    }

    private NoMain inserirEstrutura(NoMain i, int chave) {
        NoMain resultado = i;
        
        if (i == null) {
            resultado = new NoMain(chave);
        } else {
            if (chave < i.chave) {
                i.esq = inserirEstrutura(i.esq, chave);
            } else if (chave > i.chave) {
                i.dir = inserirEstrutura(i.dir, chave);
            }
            resultado = i;
        }
    ...
    public void inserir(Game x) throws Exception {
        int chave = x.EstimedOwners % 15;
        raiz = inserirGame(raiz, chave, x);
    }

    private NoMain inserirGame(NoMain i, int chave, Game x) throws Exception {
        NoMain resultado = i;
        
        if (i == null) {
            // Isso não deveria acontecer se a árvore principal foi construída corretamente
            throw new Exception("Chave não encontrada na árvore principal: " + chave);
        } else {
            if (chave < i.chave) {
                i.esq = inserirGame(i.esq, chave, x);
            } else if (chave > i.chave) {
                i.dir = inserirGame(i.dir, chave, x);
            } else {
                // Chave encontrada - insere o game na subárvore deste nó
                i.subarvore.inserir(x);
            }
            resultado = i;
        }
    ...
    private boolean pesquisar(NoMain i, String nome) {
        boolean resp = false;

        if (i != null) {
            // Primeiro busca na subárvore interna do nó atual
            resp = i.subarvore.pesquisar(nome);
            
            // Se não encontrou, busca recursivamente nas subárvores esquerda e direita
            if (!resp) {
                System.out.print("  ESQ");
                resp = pesquisar(i.esq, nome);
            }
...
class NoMain {
    ...
    public NoMain(int chave) {
        this.chave = chave;
        this.subarvore = new ArvSec();
        this.esq = null;
        this.dir = null;
    }
}
...
class Q2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //File csv=new File("C:\\Users\\samue\\OneDrive\\Área de Trabalho\\TP7\\games.csv");
        ...
        //Comandos
        ArvDeArv a=new ArvDeArv();
        while(esc.hasNextInt()){
            int idPesq=esc.nextInt();
            Game tmp=PesquisaBin(0, array.length-1, idPesq, array);
            a.inserir(tmp);
        }//Para no Fim;
        esc.nextLine();//Come fim
        while(esc.hasNext()){
        ...
        double exe=(fim-inicio)/1000_000_000;
        String matricula="801757";
        String logline=String.format(Locale.US, "%s\t%.6f\t%d%n", matricula, exe, numcomp);
        try(RandomAccessFile logbait=new RandomAccessFile(matricula+"_binaria.txt","rw")){
            logbait.write(logline.getBytes("UTF-8"));
        }
    }
}
