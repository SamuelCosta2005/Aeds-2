...
int main()
{
    ...
    while (fgets(entrada, MAX_CAMPO_TAMANHO, stdin) != NULL)
    {
...
bool pesquisarRecursivo(NoAVL *no, const char *nome, FILE *arquivoLog)
{
    ...
    else if (comparacao < 0)
    {
        ...
        return pesquisarRecursivo(no->esquerda, nome, arquivoLog);
    }
    else
    {
        printf("dir ");
        fprintf(arquivoLog, "dir ");
        return pesquisarRecursivo(no->direita, nome, arquivoLog);
    }
}
...
bool pesquisarNaArvore(ArvoreAVL *arvore, const char *nome, FILE *arquivoLog)
{
    if (arvore->raiz != NULL)
    {
        ...
        return pesquisarRecursivo(arvore->raiz, nome, arquivoLog);
    }
    return false;
}
...
int obterFatorBalanceamento(NoAVL *no)
{
    return (no == NULL) ? 0 : alturaNo(no->direita) - alturaNo(no->esquerda);
}
...
NoAVL *inserirRecursivo(NoAVL *no, Jogo *jogo)
{
    ...
    if (balanceamento > 1 && strcmp(jogo->nome, no->direita->jogo->nome) > 0)
...
void analisarECarregarJogo(Jogo *jogo, char *linha)
{
    ...
    jogo->preco = (strcmp(precoStr, "Free to Play") == 0 || strlen(precoStr) == 0) ? 0.0f : atof(precoStr);
    free(precoStr);
