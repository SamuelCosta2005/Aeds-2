...
class Q1 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while(esc.hasNextInt()){
            ...
            a.Inserir(new No(tmp));
        }
        esc.next();
        esc.nextLine();
        String tmp=esc.nextLine();
        while(!tmp.equals("FIM")){
            System.out.print(tmp+":  (Posicao: "+a.hash(tmp)+") ");
            if(a.pesquisa(tmp)){
        ...
        double executionTimeSeconds = (endTime - inicio) / 1_000_000_000.0;
        String matricula = "801757";
        String logLine = String.format(Locale.US, "%s\t%.6f\t%d%n", matricula, executionTimeSeconds, 30);
        try (RandomAccessFile logFile = new RandomAccessFile(matricula + "_hashReserva.txt", "rw")) {
            logFile.write(logLine.getBytes("UTF-8"));
        }
    }
}
